...
import Brick (App (..), BrickEvent (AppEvent, VtyEvent), EventM, ViewportScroll (vScrollToEnd), ViewportType (..), Widget (..), customMain, hLimit, modify, vLimit, viewport, viewportScroll, (<+>), (<=>))
...
userPrompt s =
    str "Enter your username:" <=> renderEditor (txt . mconcat) True (s ^. userNameEditor)
...
webSocketThread userNameMVar' bChan sendChannel = do
    ...
    runClient "127.0.0.1" 3000 ("/" <> unpack userName) clientApp
  where
    clientApp :: ClientApp ()
    clientApp conn = do
        concurrently_
